...
BLEND_PATH = SCRIPT_DIR / "carrot_normal_v001_reference_r04.blend"
FBX_PATH = EXPORT_DIR / "Crop_Carrot_Normal_v001_reference_r04.fbx"
...
def create_reference_body(material):
    ...
    mesh = bpy.data.meshes.new("Mesh_Carrot_Normal_v001_ReferenceBody_r04")
    ...
    body = bpy.data.objects.new("Crop_Carrot_Normal_v001_Body_Reference_r04", mesh)
...
def create_leaf_cluster(leaf_material, leaf_light_material, leaf_dark_material):
    top = body_center(1.0)
    crown_z = BODY_HEIGHT + 0.026

    add_uv_sphere(
        "LeafStem_GlossyRootKnot_Reference_r04",
        (top.x + 0.010, -0.012, crown_z - 0.030),
        (0.132, 0.110, 0.058),
        leaf_dark_material,
        24,
        10,
    ...
    stems = [
        (
            "LeafStem_LeftBack_Reference_r04",
            [Vector((top.x - 0.020, -0.012, crown_z - 0.042)), Vector((top.x - 0.120, -0.040, 2.31)), Vector((top.x - 0.255, -0.062, 2.42))],
            0.052,
            leaf_material,
        ),
        (
            "LeafStem_CenterTall_Reference_r04",
            [Vector((top.x + 0.006, -0.014, crown_z - 0.044)), Vector((top.x + 0.045, -0.040, 2.36)), Vector((top.x + 0.112, -0.035, 2.58))],
            0.053,
            leaf_light_material,
        ),
        (
            "LeafStem_RightTall_Reference_r04",
            [Vector((top.x + 0.032, -0.006, crown_z - 0.042)), Vector((top.x + 0.175, -0.036, 2.29)), Vector((top.x + 0.335, -0.052, 2.42))],
            0.052,
            leaf_material,
        ),
        (
            "LeafStem_RightLow_Reference_r04",
            [Vector((top.x + 0.030, 0.032, crown_z - 0.045)), Vector((top.x + 0.175, 0.078, 2.22)), Vector((top.x + 0.365, 0.104, 2.27))],
            0.049,
            leaf_dark_material,
        ),
    ]
...
def create_reference_surface_marks(groove_material, groove_light_material, highlight_material):
    create_surface_ribbon("CarrotSoftGroove_Upper_Reference_r04", 0.610, -0.185, 0.255, 0.016, -23.0, groove_material, -0.003, 0.003)
    create_surface_ribbon("CarrotSoftGroove_UpperLight_Reference_r04", 0.615, -0.192, 0.175, 0.004, -23.0, groove_light_material, -0.007, 0.001)
    create_surface_ribbon("CarrotSoftGroove_Lower_Reference_r04", 0.450, -0.225, 0.230, 0.015, -21.0, groove_material, -0.003, 0.003)
    create_surface_ribbon("CarrotSoftGroove_LowerLight_Reference_r04", 0.455, -0.232, 0.160, 0.004, -21.0, groove_light_material, -0.007, 0.001)
...
def configure_scene():
    ...
    scene.world = bpy.data.worlds.new("WorldFarm_ReferenceCarrot_World_r04")
...
def main():
    ...
    body_material = make_material("Mat_Carrot_Reference_GlossyBody_r04", (1.00, 0.41, 0.030, 1.0), 0.40, 0.50)
    groove_material = make_material("Mat_Carrot_Reference_SoftGroove_r04", (0.90, 0.27, 0.015, 1.0), 0.50, 0.16)
    groove_light_material = make_material("Mat_Carrot_Reference_GrooveHighlight_r04", (1.00, 0.67, 0.150, 1.0), 0.44, 0.26)
    highlight_material = make_material("Mat_Carrot_Reference_PaintedHighlight_r04", (1.00, 0.72, 0.22, 1.0), 0.34, 0.46)
    leaf_material = make_material("Mat_Carrot_Reference_LeafGloss_r04", (0.30, 0.72, 0.105, 1.0), 0.40, 0.44)
    leaf_light_material = make_material("Mat_Carrot_Reference_LeafLight_r04", (0.54, 0.90, 0.18, 1.0), 0.38, 0.48)
    leaf_dark_material = make_material("Mat_Carrot_Reference_LeafDark_r04", (0.11, 0.43, 0.08, 1.0), 0.46, 0.34)
    ...
    render_view("v001_reference_r04_front.png", (0.0, -6.2, 1.34), (0.02, 0.0, 1.22), 2.72)
    render_view("v001_reference_r04_side.png", (6.0, 0.0, 1.36), (0.03, 0.0, 1.22), 2.72)
    render_view("v001_reference_r04_three_quarter.png", (4.5, -5.2, 1.72), (0.03, 0.0, 1.22), 2.82)
    render_view("v001_reference_r04_icon_angle.png", (-2.9, -6.4, 1.66), (0.02, 0.0, 1.22), 2.55, 31.0)
