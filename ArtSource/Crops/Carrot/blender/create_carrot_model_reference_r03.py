...
BLEND_PATH = SCRIPT_DIR / "carrot_normal_v001_reference_r03.blend"
FBX_PATH = EXPORT_DIR / "Crop_Carrot_Normal_v001_reference_r03.fbx"
...
def body_center(t):
    bend_x = -0.012 + math.sin(t * math.pi) * 0.012 + smoothstep((t - 0.62) / 0.38) * 0.020
    return Vector((bend_x, 0.0, BODY_HEIGHT * t))


def body_squash_x(t):
    return lerp(0.94, 1.02, smoothstep(t))


def body_squash_y(t):
    return lerp(0.84, 0.92, smoothstep(t))
...
def create_reference_body(material):
    ...
    vertices.append(tuple(body_center(1.0) + Vector((0.0, 0.0, 0.045))))
    uvs.append((0.5, 1.0))
    ...
    mesh = bpy.data.meshes.new("Mesh_Carrot_Normal_v001_ReferenceBody_r03")
    ...
    body = bpy.data.objects.new("Crop_Carrot_Normal_v001_Body_Reference_r03", mesh)
...
def create_leaf_cluster(leaf_material, leaf_light_material, leaf_dark_material):
    ...
    add_uv_sphere(
        "LeafStem_GlossyRootKnot_Reference_r03",
        (top.x + 0.012, -0.012, crown_z - 0.034),
        (0.150, 0.120, 0.065),
        leaf_dark_material,
        24,
        10,
    ...
    stems = [
        (
            "LeafStem_LeftBack_Reference_r03",
            [Vector((top.x - 0.028, -0.012, crown_z - 0.045)), Vector((top.x - 0.130, -0.040, 2.39)), Vector((top.x - 0.265, -0.062, 2.50))],
            0.056,
            leaf_material,
        ),
        (
            "LeafStem_CenterTall_Reference_r03",
            [Vector((top.x + 0.004, -0.014, crown_z - 0.046)), Vector((top.x + 0.045, -0.040, 2.45)), Vector((top.x + 0.118, -0.035, 2.66))],
            0.057,
            leaf_light_material,
        ),
        (
            "LeafStem_RightTall_Reference_r03",
            [Vector((top.x + 0.038, -0.006, crown_z - 0.044)), Vector((top.x + 0.185, -0.036, 2.38)), Vector((top.x + 0.350, -0.052, 2.51))],
            0.056,
            leaf_material,
        ),
        (
            "LeafStem_RightLow_Reference_r03",
            [Vector((top.x + 0.030, 0.032, crown_z - 0.048)), Vector((top.x + 0.180, 0.078, 2.31)), Vector((top.x + 0.390, 0.104, 2.34))],
            0.053,
            leaf_dark_material,
        ),
    ]
    ...
    for name, points, radius, material in stems:
        ...
        add_uv_sphere(f"{name}_RoundedEnd", tuple(end), (radius * 0.60, radius * 0.60, radius * 0.60), material, 18, 8)
...
def create_reference_surface_marks(groove_material, groove_light_material, highlight_material):
    create_surface_ribbon("CarrotSoftGroove_Upper_Reference_r03", 0.620, -0.190, 0.275, 0.018, -23.0, groove_material, -0.003, 0.004)
    create_surface_ribbon("CarrotSoftGroove_UpperLight_Reference_r03", 0.625, -0.197, 0.190, 0.004, -23.0, groove_light_material, -0.007, 0.001)
    create_surface_ribbon("CarrotSoftGroove_Lower_Reference_r03", 0.455, -0.230, 0.245, 0.017, -21.0, groove_material, -0.003, 0.004)
    create_surface_ribbon("CarrotSoftGroove_LowerLight_Reference_r03", 0.460, -0.237, 0.175, 0.004, -21.0, groove_light_material, -0.007, 0.001)
...
def configure_scene():
    ...
    scene.world = bpy.data.worlds.new("WorldFarm_ReferenceCarrot_World_r03")
...
def main():
    ...
    body_material = make_material("Mat_Carrot_Reference_GlossyBody_r03", (1.00, 0.40, 0.030, 1.0), 0.40, 0.50)
    groove_material = make_material("Mat_Carrot_Reference_SoftGroove_r03", (0.90, 0.27, 0.015, 1.0), 0.50, 0.16)
    groove_light_material = make_material("Mat_Carrot_Reference_GrooveHighlight_r03", (1.00, 0.67, 0.150, 1.0), 0.44, 0.26)
    highlight_material = make_material("Mat_Carrot_Reference_PaintedHighlight_r03", (1.00, 0.72, 0.22, 1.0), 0.34, 0.46)
    leaf_material = make_material("Mat_Carrot_Reference_LeafGloss_r03", (0.30, 0.72, 0.105, 1.0), 0.40, 0.44)
    leaf_light_material = make_material("Mat_Carrot_Reference_LeafLight_r03", (0.54, 0.90, 0.18, 1.0), 0.38, 0.48)
    leaf_dark_material = make_material("Mat_Carrot_Reference_LeafDark_r03", (0.11, 0.43, 0.08, 1.0), 0.46, 0.34)
    ...
    render_view("v001_reference_r03_front.png", (0.0, -6.2, 1.42), (0.02, 0.0, 1.30), 2.90)
    render_view("v001_reference_r03_side.png", (6.0, 0.0, 1.44), (0.03, 0.0, 1.30), 2.90)
    render_view("v001_reference_r03_three_quarter.png", (4.5, -5.2, 1.82), (0.03, 0.0, 1.30), 2.98)
    render_view("v001_reference_r03_icon_angle.png", (-2.9, -6.4, 1.74), (0.02, 0.0, 1.30), 2.72, 31.0)
