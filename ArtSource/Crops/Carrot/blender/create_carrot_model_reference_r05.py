...
BLEND_PATH = SCRIPT_DIR / "carrot_normal_v001_reference_r05.blend"
FBX_PATH = EXPORT_DIR / "Crop_Carrot_Normal_v001_reference_r05.fbx"
...
def body_radius(t):
    ...
    max_radius = 0.560
    lower_growth = t ** 0.64
    top_taper = 1.0 - 0.82 * smoothstep((t - 0.835) / 0.165)
...
def create_reference_body(material):
    ...
    mesh = bpy.data.meshes.new("Mesh_Carrot_Normal_v001_ReferenceBody_r05")
    ...
    body = bpy.data.objects.new("Crop_Carrot_Normal_v001_Body_Reference_r05", mesh)
...
def create_tapered_stem(name, points, radius, material, tip_scale=0.52):
    radial_segments = 22
    ring_count = 34
    vertices = []
    faces = []
...
def create_leaf_cluster(leaf_material, leaf_light_material, leaf_dark_material):
    top = body_center(1.0)
    crown_z = BODY_HEIGHT + 0.016

    add_uv_sphere(
        "LeafStem_GlossyRootKnot_Reference_r05",
        (top.x + 0.008, -0.012, crown_z - 0.020),
        (0.150, 0.124, 0.070),
        leaf_dark_material,
        24,
        10,
    ...
    stems = [
        (
            "LeafStem_LeftBack_Reference_r05",
            [Vector((top.x - 0.030, -0.014, crown_z - 0.028)), Vector((top.x - 0.116, -0.040, 2.28)), Vector((top.x - 0.238, -0.058, 2.38))],
            0.052,
            leaf_material,
        ),
        (
            "LeafStem_CenterTall_Reference_r05",
            [Vector((top.x + 0.004, -0.014, crown_z - 0.030)), Vector((top.x + 0.044, -0.040, 2.35)), Vector((top.x + 0.106, -0.034, 2.54))],
            0.053,
            leaf_light_material,
        ),
        (
            "LeafStem_RightTall_Reference_r05",
            [Vector((top.x + 0.030, -0.006, crown_z - 0.028)), Vector((top.x + 0.166, -0.035, 2.27)), Vector((top.x + 0.310, -0.048, 2.39))],
            0.052,
            leaf_material,
        ),
        (
            "LeafStem_RightLow_Reference_r05",
            [Vector((top.x + 0.030, 0.030, crown_z - 0.030)), Vector((top.x + 0.164, 0.070, 2.20)), Vector((top.x + 0.330, 0.094, 2.25))],
            0.049,
            leaf_dark_material,
        ),
    ]
    ...
    for name, points, radius, material in stems:
        ...
        add_uv_sphere(f"{name}_SoftEnd", tuple(end), (radius * 0.30, radius * 0.30, radius * 0.34), material, 14, 6)
...
def create_reference_surface_marks(groove_material, groove_light_material, highlight_material):
    create_surface_ribbon("CarrotSoftGroove_Upper_Reference_r05", 0.610, -0.185, 0.255, 0.016, -23.0, groove_material, -0.003, 0.003)
    create_surface_ribbon("CarrotSoftGroove_UpperLight_Reference_r05", 0.615, -0.192, 0.175, 0.004, -23.0, groove_light_material, -0.007, 0.001)
    create_surface_ribbon("CarrotSoftGroove_Lower_Reference_r05", 0.450, -0.225, 0.230, 0.015, -21.0, groove_material, -0.003, 0.003)
    create_surface_ribbon("CarrotSoftGroove_LowerLight_Reference_r05", 0.455, -0.232, 0.160, 0.004, -21.0, groove_light_material, -0.007, 0.001)
...
def configure_scene():
    ...
    scene.world = bpy.data.worlds.new("WorldFarm_ReferenceCarrot_World_r05")
...
def main():
    ...
    body_material = make_material("Mat_Carrot_Reference_GlossyBody_r05", (1.00, 0.41, 0.030, 1.0), 0.40, 0.50)
    groove_material = make_material("Mat_Carrot_Reference_SoftGroove_r05", (0.90, 0.27, 0.015, 1.0), 0.50, 0.16)
    groove_light_material = make_material("Mat_Carrot_Reference_GrooveHighlight_r05", (1.00, 0.67, 0.150, 1.0), 0.44, 0.26)
    highlight_material = make_material("Mat_Carrot_Reference_PaintedHighlight_r05", (1.00, 0.72, 0.22, 1.0), 0.34, 0.46)
    leaf_material = make_material("Mat_Carrot_Reference_LeafGloss_r05", (0.30, 0.72, 0.105, 1.0), 0.40, 0.44)
    leaf_light_material = make_material("Mat_Carrot_Reference_LeafLight_r05", (0.54, 0.90, 0.18, 1.0), 0.38, 0.48)
    leaf_dark_material = make_material("Mat_Carrot_Reference_LeafDark_r05", (0.11, 0.43, 0.08, 1.0), 0.46, 0.34)
    ...
    render_view("v001_reference_r05_front.png", (0.0, -6.2, 1.34), (0.02, 0.0, 1.22), 2.72)
    render_view("v001_reference_r05_side.png", (6.0, 0.0, 1.36), (0.03, 0.0, 1.22), 2.72)
    render_view("v001_reference_r05_three_quarter.png", (4.5, -5.2, 1.72), (0.03, 0.0, 1.22), 2.82)
    render_view("v001_reference_r05_icon_angle.png", (-2.9, -6.4, 1.66), (0.02, 0.0, 1.22), 2.55, 31.0)
