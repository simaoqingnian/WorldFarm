...
BLEND_PATH = SCRIPT_DIR / "carrot_normal_v001_reference_r02.blend"
FBX_PATH = EXPORT_DIR / "Crop_Carrot_Normal_v001_reference_r02.fbx"
...
def body_center(t):
    bend_x = -0.020 + math.sin(t * math.pi) * 0.020 + smoothstep((t - 0.60) / 0.40) * 0.030
...
def create_reference_body(material):
    ...
    mesh = bpy.data.meshes.new("Mesh_Carrot_Normal_v001_ReferenceBody_r02")
    ...
    body = bpy.data.objects.new("Crop_Carrot_Normal_v001_Body_Reference_r02", mesh)
...
def create_leaf_cluster(leaf_material, leaf_light_material, leaf_dark_material):
    ...
    add_uv_sphere(
        "LeafStem_GlossyRootKnot_Reference_r02",
        (top.x + 0.010, -0.012, crown_z - 0.018),
        (0.205, 0.165, 0.088),
        leaf_dark_material,
        24,
        10,
    ...
    stems = [
        (
            "LeafStem_LeftBack_Reference_r02",
            [Vector((top.x - 0.030, -0.012, crown_z - 0.020)), Vector((top.x - 0.160, -0.040, 2.50)), Vector((top.x - 0.300, -0.060, 2.64))],
            0.064,
            leaf_material,
        ),
        (
            "LeafStem_CenterTall_Reference_r02",
            [Vector((top.x + 0.010, -0.014, crown_z - 0.020)), Vector((top.x + 0.060, -0.036, 2.56)), Vector((top.x + 0.150, -0.030, 2.78))],
            0.066,
            leaf_light_material,
        ),
        (
            "LeafStem_RightTall_Reference_r02",
            [Vector((top.x + 0.045, -0.006, crown_z - 0.018)), Vector((top.x + 0.220, -0.034, 2.52)), Vector((top.x + 0.390, -0.050, 2.66))],
            0.065,
            leaf_material,
        ),
        (
            "LeafStem_RightLow_Reference_r02",
            [Vector((top.x + 0.030, 0.034, crown_z - 0.022)), Vector((top.x + 0.200, 0.080, 2.43)), Vector((top.x + 0.420, 0.105, 2.47))],
            0.061,
            leaf_dark_material,
        ),
    ]
    ...
    for name, points, radius, material in stems:
        ...
        add_uv_sphere(f"{name}_RoundedEnd", tuple(end), (radius * 0.68, radius * 0.68, radius * 0.68), material, 18, 8)
...
def create_reference_surface_marks(groove_material, groove_light_material, highlight_material):
    create_surface_ribbon("CarrotSoftGroove_Upper_Reference_r02", 0.630, -0.205, 0.330, 0.022, -22.0, groove_material, -0.003, 0.006)
    create_surface_ribbon("CarrotSoftGroove_UpperLight_Reference_r02", 0.635, -0.212, 0.230, 0.005, -22.0, groove_light_material, -0.007, 0.002)
    create_surface_ribbon("CarrotSoftGroove_Lower_Reference_r02", 0.455, -0.255, 0.295, 0.020, -20.0, groove_material, -0.003, 0.005)
    create_surface_ribbon("CarrotSoftGroove_LowerLight_Reference_r02", 0.460, -0.262, 0.205, 0.005, -20.0, groove_light_material, -0.007, 0.002)
...
def configure_scene():
    ...
    scene.world = bpy.data.worlds.new("WorldFarm_ReferenceCarrot_World_r02")
...
def main():
    ...
    body_material = make_material("Mat_Carrot_Reference_GlossyBody_r02", (1.00, 0.43, 0.035, 1.0), 0.36, 0.56)
    groove_material = make_material("Mat_Carrot_Reference_SoftGroove_r02", (0.88, 0.28, 0.018, 1.0), 0.46, 0.20)
    groove_light_material = make_material("Mat_Carrot_Reference_GrooveHighlight_r02", (1.00, 0.70, 0.170, 1.0), 0.42, 0.32)
    highlight_material = make_material("Mat_Carrot_Reference_PaintedHighlight_r02", (1.00, 0.74, 0.24, 1.0), 0.30, 0.56)
    leaf_material = make_material("Mat_Carrot_Reference_LeafGloss_r02", (0.30, 0.74, 0.105, 1.0), 0.38, 0.48)
    leaf_light_material = make_material("Mat_Carrot_Reference_LeafLight_r02", (0.55, 0.93, 0.18, 1.0), 0.36, 0.52)
    leaf_dark_material = make_material("Mat_Carrot_Reference_LeafDark_r02", (0.11, 0.45, 0.08, 1.0), 0.44, 0.38)
    ...
    render_view("v001_reference_r02_front.png", (0.0, -6.2, 1.48), (0.02, 0.0, 1.34), 3.05)
    render_view("v001_reference_r02_side.png", (6.0, 0.0, 1.50), (0.03, 0.0, 1.34), 3.05)
    render_view("v001_reference_r02_three_quarter.png", (4.5, -5.2, 1.88), (0.03, 0.0, 1.34), 3.12)
    render_view("v001_reference_r02_icon_angle.png", (-2.9, -6.4, 1.80), (0.02, 0.0, 1.34), 2.85, 31.0)
